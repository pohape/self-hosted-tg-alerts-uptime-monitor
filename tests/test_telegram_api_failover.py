...
class TokenIsNeverPrintedTest(unittest.TestCase):
    ...
    def test_failover_warning_masks_the_token(self):
        ...
        # Токен правдоподобной ФОРМЫ, но с заведомо выдуманным номером бота:
        # односимвольный `T` из общего стенда не годится (маскировка съела бы
        # каждую `T` в тексте, и тест мерил бы собственную выдумку), а брать
        # форму у настоящего бота нельзя — сканер секретов GitHub опознаёт её
        # по шаблону и поднимает тревогу, даже когда секретная половина
        # выдумана. Номер бота секретом не является, но светить его в публичном
        # репозитории незачем.
        token = '1234567890:AAHFAKEfakeFAKEfakeFAKEfakeFAKEfake'
